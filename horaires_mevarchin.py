...
def get_molad_datetime(jc):
    molad = jc.molad()
    # Le molad est exprimé selon le calendrier juif. Il faut trouver la date grégorienne du molad.
    # On prend le jour du molad (dimanche=lundi=0/1 etc.) par rapport au début du mois hébraïque.
    # Trouve la date du 1er du mois
    jewish_year = jc.jewish_year
    jewish_month = jc.jewish_month
    # Crée un JewishCalendar pour le 1er du mois
    first_of_month = JewishCalendar(jewish_year, jewish_month, 1)
    gregorian_first = datetime(first_of_month.gregorian_year, first_of_month.gregorian_month, first_of_month.gregorian_day)
    # Ajoute le nombre de jours du molad (molad_day), puis l'heure, minutes, chalakim (1 chelek = 1/18 minute)
    dt = gregorian_first + timedelta(days=molad.molad_day)
    dt += timedelta(hours=molad.molad_hours, minutes=molad.molad_minutes, seconds=molad.molad_chalakim * 10 / 3)
    return dt
...
def calculate_last_kiddush_levana_date(gregorian_date):
    jc = JewishCalendar(datetime.combine(gregorian_date, datetime.min.time()))
    molad_dt = get_molad_datetime(jc)
    latest_time = molad_dt + timedelta(days=14, hours=18)
            ...
            with Image.open(template) as img:
               ...
               # On inverse la partie hébreu mais PAS la date                    
                ...
                if not is_mevarchim:
                    ...
                    if shabbat_date < latest_kiddush_levana.date():
                        message = f"תאריך אחרון לאמירת ברכת הלבנה: {latest_kiddush_levana.strftime('%d/%m/%Y')}"
                        draw.text(
                            (200, img_h - 180),  # Position en bas de l'image
                            message,
                            fill="blue",
                            font=font
                        )
                    ...
                    if shabbat_date >= start_kiddush_levana.date():
                        message2 = f"זמן התחלה לאמירת ברכת הלבנה: {start_kiddush_levana.strftime('%d/%m/%Y')}"
                        draw.text(
                            (200, img_h - 140),  # Position au-dessus de la ligne précédente
                            message2,
                            fill="blue",
                            font=font
                        )
