...
class ShabbatScheduleGenerator:
    ...
    def calculate_times(self, shabbat_start, shabbat_end):
        """
        Calcule tous les horaires du Shabbat, y compris :
        - arvit (tsé hakochavim moyen),
        - mincha_midweek (coucher min -17 min),
        - arvit_motsach (fin du Shabbat).
        """
        # Heures de début / fin en minutes depuis minuit
        start_m = shabbat_start.hour * 60 + shabbat_start.minute
        end_m   = shabbat_end.hour   * 60 + shabbat_end.minute

        times = {
            "mincha_kabbalat":  start_m,
            "shir_hashirim":    self.round_to_nearest_five(start_m - 10),
        ...
        # Shiur Rav, Min'ha 2, Parasha Hashavua
        times["mincha_2"] = self.round_to_nearest_five(end_m - 90)
        ...
        # Tsé hakochavim (arvit)
        sunday   = shabbat_start.date() + timedelta(days=2)
        thursday = sunday + timedelta(days=4)
        s_sun = sun(self.ramat_gan.observer, date=sunday,   tzinfo=self.ramat_gan.timezone)
        s_thu = sun(self.ramat_gan.observer, date=thursday, tzinfo=self.ramat_gan.timezone)

        def to_min(dt):
            return dt.hour*60 + dt.minute if dt else None

        dusk_sun = to_min(s_sun.get("dusk"))
        dusk_thu = to_min(s_thu.get("dusk"))
        if dusk_sun and dusk_thu:
            avg = (dusk_sun + dusk_thu)//2
            arvit_ts = self.round_to_nearest_five(avg)
            earliest = min(dusk_sun, dusk_thu)
            if earliest - arvit_ts > 3:
                arvit_ts = self.round_to_nearest_five(earliest - 3)
            times["arvit"] = arvit_ts
        else:
            times["arvit"] = None

        # Min'ha Bineyim (mincha_midweek)
        sunset_sun = to_min(s_sun.get("sunset"))
        sunset_thu = to_min(s_thu.get("sunset"))
        if sunset_sun and sunset_thu:
            base = min(sunset_sun, sunset_thu) - 17
            times["mincha_midweek"] = self.round_to_nearest_five(base) if base>0 else None
        else:
            times["mincha_midweek"] = None
    ...
    def get_next_shabbat_time(self, current_shabbat_date):
        """
        Retourne (date_str, time_str) du prochain Shabbat dans yearly_shabbat_data.
        Accepte current_shabbat_date en date ou datetime.
        """
        try:
            # Si on reçoit une date pure, on la convertit en datetime
            if isinstance(current_shabbat_date, datetime):
                current_dt = current_shabbat_date
            else:
                current_dt = datetime(
                    current_shabbat_date.year,
                    current_shabbat_date.month,
                    current_shabbat_date.day
                )

            change_time_date = datetime(2025, 3, 27)
            last = None
            for sh in self.yearly_shabbat_data:
                sh_date = datetime.strptime(sh["day"], "%Y-%m-%d %H:%M:%S")
                if sh_date > current_dt:
                    # On gère l'ancien changement d'heure si besoin
                    entry = last if sh_date > change_time_date and last else sh
                    h, m = map(int, entry["כנסית שבת"].split(":"))
                    total_minutes = h * 60 + m
                    # C’est l’heure de mincha en semaine qu’on retourne
                    return sh_date.strftime("%d/%m/%Y"), \
                           self.format_time(self.round_to_nearest_five(total_minutes))
                last = sh_date
            return None, None
        except Exception as e:
    ...
    def create_image(self, times, parasha, parasha_hebrew, shabbat_end, candle_lighting, shabbat_date, is_mevarchim=False):
        """
        Crée l’image des horaires en utilisant :
        - un template spécial si Mevarchim,
        - puis dessine tous les horaires, y compris les arvit et mincha_midweek.
        """
        tpl = self.template_path
        special = tpl.parent / "template_rosh_hodesh.jpg"
        if is_mevarchim and special.exists():
            tpl = special

        with Image.open(tpl) as img:
            draw = ImageDraw.Draw(img)
            font = ImageFont.truetype(str(self.font_path), 30)
            x = 120

            # Les 9 horaires "classiques"
            positions = [
                ("shir_hashirim",400), ("mincha_kabbalat",475),
                ("shacharit",510),     ("mincha_gdola",550),
                ("tehilim",590),       ("shiur_nashim",630),
                ("parashat_hashavua",670), ("shiur_rav",710),
                ("mincha_2",750)
            ]
            for key,y in positions:
                val = self.format_time(times[key])
                # hiver/été pour tehilim
                if key=="tehilim" and self.season=="summer":
                    val = "17:00/"+val
                draw.text((x,y), val, fill="black", font=font)

            # --- fin du Shabbat (arvit_motsach) en NOIR ---
            draw.text((x, 830), self.format_time(times["arvit_motsach"]), fill="black", font=font)

            # --- arvit_midweek (tsé) en VERT ---
            draw.text((x, 990), self.format_time(times["arvit"]), fill="green", font=font)

            # --- mincha_midweek (מנחה ביניים) en VERT ---
            draw.text((x, 950), self.format_time(times["mincha_midweek"]), fill="green", font=font)
            # nom de la parasha
            ...
            # sauvegarde
            out = self.output_dir / f"horaires_{self.sanitize_filename(parasha)}.jpeg"
            img.save(out)
