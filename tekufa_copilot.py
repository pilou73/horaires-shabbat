...
class ShabbatScheduleGenerator:
    ...
    def update_excel_with_mevarchim_column(self, excel_path: Path):
        ...
        if excel_path.exists():
            ...
            df = pd.DataFrame(self.yearly_shabbat_data)
            df["day"] = pd.to_datetime(df["day"], format="%Y-%m-%d %H:%M:%S").dt.date
            # --- Ajout Sheet1 à la création si vide ---
            if not df.empty:
                sheets["Sheet1"] = df.iloc[[0]].copy()
            else:
        ...
        # Supprimer les colonnes horaires intermédiaires si elles existent déjà
        for col in ["שקיעה Dimanche", "שקיעה Jeudi", "מנחה ביניים", "ערבית ביניים"]:
