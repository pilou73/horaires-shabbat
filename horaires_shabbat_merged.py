            ...
            for shabbat in self.yearly_shabbat_data:
                ...
                if shabbat_date > current_date:
                    if shabbat_date > change_time_date and last_shabbat:
                        shabbat = last_shabbat  # Utiliser le Chabbat précédent si le prochain tombe après le changement d'heure
...
# Configuration de l'API Hebcal pour récupérer l'heure du coucher du soleil
def get_sunset_time(date, location="Jerusalem"):
    url = f"https://www.hebcal.com/zmanim?cfg=json&city={location}&date={date}"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        return data.get("sunset")  # Récupération de l'heure du coucher du soleil
    return None

# Lecture du fichier Excel
file_path = "horaires_shabbat.xlsx"
df = pd.read_excel(file_path, sheet_name="שבתות השנה")

# Ajout de la colonne שקיעה (coucher du soleil)
df["שקיעה"] = df["Date"].apply(lambda d: get_sunset_time(d.strftime("%Y-%m-%d")))
df.to_excel(file_path, sheet_name="שבתות השנה", index=False)

# Gestion des horaires
tehilim_1 = "13:45"
tehilim_2 = "17:00"
tehilim_display = f"{tehilim_1}/{tehilim_2}"

# Calcul de l'heure de Minha en semaine en fonction de l'entrée de Chabbat
next_shabbat_entry = datetime.datetime.strptime("18:23", "%H:%M")  # Exemple
weekday_mincha = next_shabbat_entry - datetime.timedelta(minutes=40)
weekday_mincha = weekday_mincha.replace(minute=(weekday_mincha.minute // 5) * 5)

# Calcul d'Arvit après Chabbat
shabbat_end = datetime.datetime.strptime("19:45", "%H:%M")  # Exemple
arvit_time = shabbat_end + datetime.timedelta(minutes=10)
arvit_time = arvit_time.replace(minute=(arvit_time.minute // 5) * 5)

# Calcul de Minha 2 et du cours du Rav
minha_2 = weekday_mincha - datetime.timedelta(minutes=90)
cours_rav = weekday_mincha - datetime.timedelta(minutes=45)

# Affichage des résultats
print("Tehilim:", tehilim_display)
print("Horaire Minha en semaine:", weekday_mincha.strftime("%H:%M"))
print("Horaire Arvit après Chabbat:", arvit_time.strftime("%H:%M"))
print("Minha 2:", minha_2.strftime("%H:%M"))
print("Cours Rav:", cours_rav.strftime("%H:%M"))
